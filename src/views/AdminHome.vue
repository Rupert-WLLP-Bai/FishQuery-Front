    ...
    <div class="admin-home">
        ...
        <el-row>
            <el-col :span="24">
                <el-card>
                    <h2>鱼类种类表</h2>
                    <el-table :data="fishTypes" style="width: 100%">
        ...
        <el-row>
            <el-col :span="24">
                <el-card>
                    <h2>用户上传的图审核</h2>
                    <el-table :data="pendingImages" style="width: 100%">
                        <el-table-column prop="user_id" label="ID" width="50" />
                        <el-table-column prop="image" label="图片">
                            <template v-slot="scope">
                                <el-image :src="scope.row.image_url" style="width: 100px; height: 100px" />
                            </template>
                        </el-table-column>
...
export default defineComponent({
    ...
    setup() {
        ...
        const approveImage = async (id: number) => {
            try {
                await api.post('/record/records/approve', {
                    'record_id': id,
        ...
        const rejectImage = async (id: number) => {
            try {
                await api.post('/record/records/reject', {
                    'record_id': id,
                    'reviewed_by': userStore.user.id,
                    'feedback': 'Rejected'
                });
        ...
        const uploadFishType = async (name_cn: string, name_latin: string, description: string) => {
            try {
                await api.post('/pictures/add_fish_type', {
                    'name_cn': name_cn,
